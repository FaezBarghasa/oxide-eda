...
- [x] Current schematic runtime callsites mapped in `oxide-app`.
- [x] `oxide-renderer::schematic` contract and dirty-family behavior reviewed.
...
- [x] Task 03: Canvas render path switched to `oxide-renderer` scene flow. Evidence: [logs/milestone-f-task-03-canvas-cutover.md](../logs/milestone-f-task-03-canvas-cutover.md)
...
- [ ] `cargo test -p oxide-renderer` passes.
- [x] Targeted `oxide-app` regression test target compiles (`--no-run`).
- [x] Focused runtime parity tests pass (`cargo test -p oxide-app schematic_runtime::tests`).
