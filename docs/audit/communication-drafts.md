...
We've completed the Apache-clean remediation. The main oxide repo no
longer contains KiCad-derived code; KiCad I/O lives in a separate
GPL-3.0-or-later companion tool at https://github.com/alplabai/oxide-kicad-import.

Specifically:

- `kicad-parser` and `kicad-writer` crates moved to the companion repo
  under GPL-3.0-or-later (no longer in the main workspace).
- `oxide-output/src/netlist/kicad_sexpr.rs` (KiCad netlist exporter)
  moved out of the main repo.
- KiCad-flavoured types in `oxide-types` removed and replaced with
  Oxide-curated designs:
  - `PinElectricalType` (12-variant, KiCad-shaped) → `PinDirection`
    (14 variants — adds `GroundReference`, `Differential`, `Clock` as
    Oxide-original; rationale in `crates/oxide-types/docs/pin-design.md`).
  ...
  - KiCad markup parser (`~{}` / `^{}` / `_{}` curly-brace syntax) →
    Markdown-extension `parse_oxide_markup` (`**bold**`, `*italic*`,
...
- v0.9.0 ships the first Apache-clean main binary alongside
  `oxide-kicad-import` v0.1.0 for users with existing KiCad projects.
...
**Posting:** Comment on https://github.com/alplabai/oxide/issues/62
once v0.9.0 ships and the companion repo is public.

---

## 2. oxide.dev website update (separate website repo at `../oxide-website`)
...
Oxide reads and writes its own native `.snxsch` / `.snxpcb` formats.
...
`.kicad_pro` projects to Oxide format one-way. Run it once; open the
result in Oxide from then on.
```
...
Per [issue #62](https://github.com/alplabai/oxide/issues/62), the main
oxide repo is now Apache-2.0 clean. KiCad import / export moves to a
separate optional companion tool at
https://github.com/alplabai/oxide-kicad-import (GPL-3.0-or-later).
...
2. If you've been opening `.kicad_sch` files directly, the new flow is:
   download `oxide-kicad-import` from
   https://github.com/alplabai/oxide-kicad-import/releases, run it
   once against your `.kicad_pro`, then open the resulting `.snxprj`
   in Oxide. Conversion is one-way and keeps your originals intact.
3. Native `.snxsch` / `.snxpcb` formats are now the canonical Oxide
   formats — TOML envelope + TSV bulk blocks, line-diffable in git,
   ~5× smaller than JSON.

Background and rationale are in [docs/LICENSING.md](https://github.com/alplabai/oxide/blob/dev/docs/LICENSING.md).
...
**Apply by:** sign in to GitHub Discussions for `alplabai/oxide`,
create a new "Announcements" discussion, paste, pin.

---

## 4. Discord announcement (if there's a Oxide Discord)

Short version (Discord posts read shorter than GitHub):

```
📣 Oxide v0.9 ships Apache-clean

Per issue #62, the main repo is now Apache-2.0 clean. KiCad I/O
moves to an optional companion tool: oxide-kicad-import (GPL-3.0).

What this means:
• v0.7/v0.8 binaries flagged superseded — please install v0.9+.
• Migrating from KiCad? Download oxide-kicad-import, run once,
  open the resulting .snxprj in Oxide.
• Native .snxsch / .snxpcb formats are the canonical Oxide
  formats — line-diffable, ~5× smaller than JSON.

Details: https://github.com/alplabai/oxide/blob/dev/docs/LICENSING.md
Companion: https://github.com/alplabai/oxide-kicad-import
```
...
## 5. Email to oxide.dev mailing list / newsletter (if one exists)
...
2. Tag v0.1.0 on the companion repo (`oxide-kicad-import`); run its
...
5. Update oxide.dev hero + licensing page; deploy.
